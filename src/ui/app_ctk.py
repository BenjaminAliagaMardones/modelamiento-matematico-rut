...
class App(ctk.CTk):
    def __init__(self):
        ...
        self._entries_limites: dict[str, ctk.CTkEntry] = {}
        self._construir()
    ...
    def _construir(self) -> None:
        ...
        ctk.CTkLabel(barra, text="RUT:", font=("", 14)
                     ).pack(side="left", padx=(12, 6))
        self._entry_rut = ctk.CTkEntry(
            barra, width=200, placeholder_text="12345678-5")
        ...
        self._txt_canonica = self._textbox(
            self._tabs.tab("Clasificación / Canónica"))
        self._construir_tab_grafica(self._tabs.tab("Gráfica"))
        self._txt_tramos = self._textbox(self._tabs.tab("Límites"))

        padre_tramos = self._tabs.tab("Gráfica Tramos")
        self._btn_validar_lim = ctk.CTkButton(
            padre_tramos, text="Validar Límites", command=self._validar_limites_tramos
        )
        # Esto lo fija abajo del todo, independientemente de lo que pase en el canvas
        self._btn_validar_lim.pack(side="bottom", fill="x", pady=10, padx=10)
    ...
    def _construir_tab_grafica(self, padre) -> None:
        ...
        self._lbl_grafica = ctk.CTkLabel(
            izquierda, text="Ingrese un RUT y presione Analizar.", font=("", 13))
        ...
        derecha = ctk.CTkScrollableFrame(
            contenedor, label_text="Elementos de la cónica")
        ...
        self._lbl_resultado_val = ctk.CTkLabel(
            derecha, text="", font=("", 13, "bold"))
    ...
    def _analizar_elementos(self, datos_canonica, datos_grafica):
        ...
        self._elementos_correctos = {
            campo: "No aplica" for campo in CAMPOS_ELEMENTOS}
        ...
        if tipo == "elipse":
            ...
            self._elementos_correctos["Eje mayor / transverso"] = fmt_dec(
                2 * a)
            ...
            self._elementos_correctos["Eje mayor / transverso"] = fmt_dec(
                2 * dx_abs)
            self._elementos_correctos["Eje menor / conjugado"] = fmt_dec(
                2 * dy_abs)
            ...
            # Parábola no tiene ambos ejes
            self._elementos_correctos["Eje mayor / transverso"] = "No aplica"
    ...
    def _analizar(self) -> None:
        ...
        try:
            ...
            self._limites_correctos = {
                "Límite por la izquierda": str(res_limites.get("limite_izquierdo")),
                "Límite por la derecha": str(res_limites.get("limite_derecho")),
                "Existencia del límite": "existe" if res_limites.get("existe_limite") else "no existe",
                "Valor en el punto": "n/a",
                "Continuidad": "continua" if "continua" in str(res_limites.get("conclusion")).lower() else "discontinua",
                "Tipo de discontinuidad": "infinita" if "infinita" in str(res_limites.get("conclusion")).lower() else
                ("salto" if "salto" in str(res_limites.get(
                    "conclusion")).lower() else "removible"),
            }
    ...
    def _dibujar(self, datos: dict) -> None:
        ...
        while k * paso <= xmax + paso:
            ...
            if xmin <= gx <= xmax:
                self._canvas.create_line(
                    px(gx), 0, px(gx), LIENZO, fill=COLOR_GRILLA)
            gy = k * paso
            if ymin <= gy <= ymax:
                self._canvas.create_line(
                    0, py(gy), LIENZO, py(gy), fill=COLOR_GRILLA)
            k += 1

        # ejes
        if ymin <= 0 <= ymax:
            self._canvas.create_line(
                0, py(0), LIENZO, py(0), fill=COLOR_EJE, width=2)
        if xmin <= 0 <= xmax:
            self._canvas.create_line(px(0), 0, px(
                0), LIENZO, fill=COLOR_EJE, width=2)

        # curva (puntos densos calculados a mano)
        for (x, y) in datos["puntos"]:
            cx, cy = px(x), py(y)
            self._canvas.create_oval(
                cx - 1, cy - 1, cx + 1, cy + 1, fill=COLOR_CURVA, outline="")
        ...
        if xmin <= hx <= xmax and ymin <= ky <= ymax:
            cx, cy = px(hx), py(ky)
            self._canvas.create_line(
                cx - 6, cy, cx + 6, cy, fill=COLOR_CENTRO, width=2)
            self._canvas.create_line(
...
# ------------------------------------------------------- dibujo tramos
    ...
    def _dibujar_tramos(self, datos: dict) -> None:
        if not hasattr(self, "_tramos_interfaz_creada"):
            ...
            self._canvas_tramos.pack(padx=10, pady=10)

            # Derecha: Scrollable con los inputs y el BOTÓN
            derecha = ctk.CTkScrollableFrame(
                contenedor, label_text="Análisis de límites")
            derecha.pack(side="right", fill="both", expand=True)

            for nombre in CAMPOS_LIMITES:
                fila = ctk.CTkFrame(derecha, fg_color="transparent")
                fila.pack(fill="x", padx=8, pady=4)
                ...
                entrada = ctk.CTkEntry(fila, placeholder_text="...")
                entrada.pack(side="left", fill="x", expand=True)
                self._entries_limites[nombre] = entrada
        ...
        if xmin <= 0 <= xmax:
            self._canvas_tramos.create_line(
                px(0), 0, px(0), LIENZO, fill=COLOR_EJE, width=2)
        ...
        self._tramos_interfaz_creada = True

    def _validar_limites_tramos(self):
        if not hasattr(self, "_limites_correctos"):
            self._estado.configure(
                text="Primero genera la función por tramos", text_color="#ff6b6b")
            return

        errores = 0

        for campo, entrada in self._entries_limites.items():
            val_user = normalizar_y_evaluar(entrada.get())
            val_real = normalizar_y_evaluar(
                str(self._limites_correctos.get(campo, "")))

            if val_user == val_real:
                entrada.configure(fg_color="#1e3a1e")
            else:
                entrada.configure(fg_color="#3a1e1e")
                errores += 1

        if errores == 0:
            self._estado.configure(
                text="Límites correctos", text_color="#5dd39e")
        else:
            self._estado.configure(
                text=f"{errores} errores en límites", text_color="#ff6b6b")

    def _limpiar_resultados(self) -> None:
        # ... (tu código actual)

        # Limpiar entradas de límites
        for entrada in self._entries_limites.values():
            entrada.delete(0, "end")
            entrada.configure(fg_color=("#F9F9FA", "#343638"))
